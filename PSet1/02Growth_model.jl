...
@with_kw struct Primitives
    ...
    z_grid::Array{Float64,1} = collect(range(Zb, length = nz, stop = Zg)) #productivity grid
    Π::Matrix{Float64}       = [0.926 0.074; 0.023 0.977]  
end
...
function Initialize()
    ...
    pol_func = zeros(prim.nk, prim.nz) #initial porlicy function guess
...
function Bellman(prim::Primitives,res::Results)
    ...
    #budget   = [z_grid[1].*k_grid.^α + (1-δ).*k_grid  z_grid[2].*k_grid.^α + (1-δ).*k_grid]
    #budget = z_grid'*k_grid.^α + (1-δ).*k_grid #budget
    for z_index =1:nz
        ...
        for k_index = 1:nk
            ...
            candidate_max = -Inf #bad candidate max
            budget = zy*k^α + (1-δ)*k #budget
            for kp_index = 1:nk #loop over possible selections of k', exploiting monotonicity of policy function
                c = budget - k_grid[kp_index] #consumption given k' selection
                if c>0 #check for positivity
                    val = log(c) + β*sum(val_func[kp_index,:].*Π[z_index,:]) #compute value
