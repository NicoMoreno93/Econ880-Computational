...
function Initialize()
    ...
    EC        = ones(1,1).*ce
    LMC       = ones(1,1).*0
    res = Results(N_S, n_D,L_D, W_func,x_prime, mu_mat, M_entry,P_new,EC, LMC, Π, π_s) #initialize results struct
    prim, res #return deliverables
end

function Value_Function(prim::Primitives, res::Results)
    @unpack β, ns, θ, ce, cf, F_s, s_grid= prim
    @unpack n_D, P_new, W_func, π_s, x_prime = res
    # Static Problem:
    ...
    for sss =1:ns
        E_W[sss,1]   = sum(F_s[sss,:]'*W_func) # Compute conditional expectations
    end
    W_func0       = π_s + β.*E_W # If the firm stays
    W_func1       = π_s   # If the firm exits next period
    W_func_prime  = maximum(hcat(W_func0,W_func1),dims= 2) # Here's the DISCRETE CHOICE
    x_aux         = argmax(hcat(W_func0,W_func1),dims=2) # Find out what's the decision
    res.x_prime   = [i[2] for i in x_aux] .-1 # cartesian_indices = max_index0[1]
    return W_func_prime
end

function Bellman_Operator(prim::Primitives, res::Results; tol::Float64 = 1e-8, err::Float64 = 100.0)
    # Initialize while
    ii = 0 #counter
    while err>tol #begin iteration
        W_func_prime = Value_Function(prim, res) # Find New VF
        # Sup Norm:
        err = norm(W_func_prime.-res.W_func,Inf) # Inf Indicates sup norm (Max of Abs term by term)
        res.W_func = W_func_prime # Update VF
        ii+=1
    end
...
function Entry_Condition(prim::Primitives, res::Results)
    ...
    @unpack ce, ν_s = prim
    @unpack P_new, W_func = res
    EC1 = ν_s*W_func/P_new .- ce
    return EC1
end

function Finding_Price(prim::Primitives, res::Results; tol::Float64 = 1e-6, err::Float64 = 100.0)
    @unpack A_ = prim
    @unpack EC, W_func, Π, P_new = res
    ii= 1 # Counter
    # Define the initial bounds for the bisect algorithmÑ
    P_auxUB = P_new+0.1  # For this P, EC1 is positive
    P_auxLB = P_new/2    # For this P, EC1 is negative, so Root is inside 
    while err > tol
...
function New_Trans_Mat(prim::Primitives, res::Results)
    @unpack F_s, ν_s = prim
...
function Finding_Entry(prim::Primitives, res::Results; tol::Float64 = 1e-5, err::Float64 = 100.0)
    @unpack M_entry = res
    ii= 1 # Counter
    # Define the initial bounds for the bisect algorithmÑ
    M_auxLB = M_entry - 0.1 # For this M, LMC is negative
    M_auxUB = M_entry + 3  # For this M, LMC is Positve, so Root is inside interval 
    while err > tol
...
function HR_Solve(prim::Primitives, res::Results)
