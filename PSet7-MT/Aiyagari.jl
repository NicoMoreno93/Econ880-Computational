...
function Bellman(prim::Primitives, res::Results)
    ...
    interp1 = interpolate(V[:, 1], BSpline(Linear())) # Performs the interpolation, but this is only for gaps "inside" the grid
    ...
    for (ϵ_index, ϵ) in enumerate(ϵ_grid)
        ...
        for (k_index, k) in enumerate(k_grid)
            ...
            res = optimize(obj, 0.0, budget) #1st arg = V,2nd=lower bound,3rd=upper bound
